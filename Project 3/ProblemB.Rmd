...
```
We want test the hypothesis that $\beta_1=\beta_2=\beta_3,$ i.e. that the expected bilirubin levels are actually the same for all individuals. To check this, we use the F-test with the reported F statistic from the $\texttt{summary}$, `r Fval`. The reported p-value of this F-statistic is 0.039, and thus we can reject the hypothesis that $\beta_1=\beta_2=\beta_3$ at significance level 5$\%$. Alternatively, we can argue that the 95$\%$ quantile of the F-distribution with 2 and 26 degrees of freedom is `r qf(.95, df1=2, df2=26)`, which is less than $\texttt{Fval}$, which gives the same conclusion. 
```{r}
#  defining function PermTest():
PermTest <- function(bilirubin){
  # create permutation
  perm.df <- data.frame(meas=sample(bilirubin$meas), pers = bilirubin$pers)
  # perform regresiion on permutation
...
```
The p-value is 0.042, so at a significance level 5$\%$ we reject the hypothesis that the bilirubin levels are the same for all individuals. 
