...
# ( var(beta_{LS,1}), var(beta_{LS,2}) )
betaLS.var = c(var(betas.boot[1,]), var(betas.boot[2,]))
# ( var(beta_{LA,1}), var(beta_{LA,2}) )
betaLA.var = c(var(betas.boot[3,]), var(betas.boot[4,]))    

cat("Variance of LS estimator:", betaLS.var)
cat("\nVariance of LA estimator:", betaLA.var)

# ( mean(beta_{LS,1}), mean(beta_{LS,2}) )
betaLS.mean = c(mean(betas.boot[1,]), mean(betas.boot[2,]))
# ( mean(beta_{LA,1}), mean(beta_{LA,2}) )
betaLA.mean = c(mean(betas.boot[3,]), mean(betas.boot[4,]))   
