...
if(TRUE)
{
  ...
  curve(dnorm(x, mean=mymean, sd=sd)/dnorm(mymean, mean=mymean, sd=sd), 
        from = 0, to = 1, add=T, col = 3, lwd=3)
  legend("bottomleft", c("target","indep proposal F=1", "indep proposal F=2"), 
         lwd=3,col=c(1,2,3))
}
...
xsamplesF10 <- mcmc_indep(M=M, F=10, y=data)
xsamplesRW <- mcmc_rw(M=M, y=data)

## some plots
# Independence proposal F=1
par(mfrow=c(3,3))
# traceplot
plot(xsamplesF1, type="l")
# autocorrelation plot
acf(xsamplesF1)
# histogram
hist(xsamplesF1, nclass=100, xlim=c(0.4,0.8), prob=T)

# Independence proposal F=10
plot(xsamplesF10, type="l")
acf(xsamplesF10)
hist(xsamplesF10, nclass=100, xlim=c(0.4,0.8), prob=T)

# RW proposal
plot(xsamplesRW, type="l")
acf(xsamplesRW)
hist(xsamplesRW, nclass=100, xlim=c(0.4,0.8), prob=T)
