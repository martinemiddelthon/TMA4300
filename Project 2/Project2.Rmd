...
We consider the built-in coal-mining data set in $\texttt{R}$ and we adopt a hierarchical Bayesian model to analyse the data set. We assume that the coal-mining disasters follow an inhomogeneous Ppisson process with intensity function $\lambda(t).$ Furthermore, we assume that $\lambda(t)$ is piecewise constant with $n$ break points, where $t_0$ and $t_{n+1}$ are the start- and end-times of the dataset, respectively, and $t_k;\;k=1,\ldots,n$ are break-points in the intensity function:
$$
\lambda = \begin{cases}
...
\end{cases}
$$
Our model parameters are then $(\beta, t_1,\ldots,t_n, \lambda_0, \ldots, \lambda_n)$.
...
```
We observe that when we take the burn-in into consideration, the algoritm seems to produce a good fit with all values of $\sigma.$
