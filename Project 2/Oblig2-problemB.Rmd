...
$$
  ...
  \end{aligned}\quad,
$$
where $|\cdot|$ denotes the determinant and we have used that $|\mathbf{A}^{-1}|=\frac{1}{|A|}$. We use a grid of values for $\theta$ and calculate the posterior marginal. The plot below shows the result, and it seems to be in concordance with the MCMC estimate displayed by the histogram.
...
thetas = seq(from = 0, to = 9, by = 0.1)
pi = pi_theta_y(thetas, y)
plot(thetas, pi, xlab = bquote(theta), ylab = bquote(pi))
...
```{r}
set.seed(0)
pi_etai_y_theta = function(etai.vec, theta, y) {
  i = 10
  T = length(y)
  Q = make.Q(T, theta)
  A = Q + diag(T)
  ...
  mean = y[i] / a_ii
  var = 1 / a_ii
  pi = dnorm(etai.vec, mean = mean, sd= sqrt(var))
  return(pi)
}


pi_etai_y = function(y,theta.grid, eta.grid) {
  sums = rep(0, length(eta.grid))
  step = theta.grid[2]-theta.grid[1]
  theta_y = pi_theta_y(theta.grid, y)
  for(j in (1:length(theta.grid))) {
    theta = theta.grid[j]
    sums = sums + pi_etai_y_theta(eta.grid, theta, y) * theta_y[j] * step
  }
  return(sums)
}
...
thetas = seq(from = 0, to = 9, by = 0.1)
eta.grid = seq(-1,1,0.01)
etai_y = pi_etai_y(y,thetas,eta.grid)

plot(eta.grid,etai_y)
...
plot(result1$summary.random$t$mean)
plot(result1$marginals.hyperpar$`Precision for t`, xlim =c(0,9))
summary(result1)
plot(result1$marginals.random$t$index.10,xlim=c(-4,4))
```
