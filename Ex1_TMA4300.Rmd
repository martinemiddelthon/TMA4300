...
```
For the 10000 samples from the $Gamma(4,1)$ distribution we make a histogram and compute the empirical and theoretical means and variances. The histogram looks as expected and the empirical mean and variance coincide with the theoretical values.

We observe that the number of tries to generate 10000 samples as a function of $\alpha$ has an approximately logarithmic shape.
...
```
We generate 10000 samples from the gamma distribution with different parameters and compute the empirical and theoretical means and variances. The empirical mean and variance seems to coincide pretty well with the theoretical values in all cases.
...
$$\hat{\mu} = \frac{1}{N}\sum_{i=1}^{N}\theta_i,$$ where $\theta_i$ are sampeled from $f(\theta\mid\mathbf{y})$ $N$ repetitions. 
...
```{r}
multbin = multbinom(10000)
multbin.tries = multbin[1]
multbin.sample = multbin[-1]
...
We have $N$ samples og $\theta_i \sim f(\theta\mid\mathbf{y})|_{f(\theta) = \text{Beta}(1,1)},$ and we want to find the expectation 
