...
which gives the prosterior distribution of $\theta$ given $\mathbf{y}$
$$f(\theta\mid\mathbf{y}) \propto f(\mathbf{y}\mid\theta)f(\theta) \propto f(\theta\mid\mathbf{y})|_{f(\theta) = \text{Beta}(1,1)}\cdot\text{Beta}(\theta;1,5).$$
Then $$\mathbf{E}[\theta\mid\mathbf{y}] = \int_0^1\theta\cdot\text{Beta}(\theta;1,5)\cdot f(\theta\mid\mathbf{y})|_{f(\theta) = \text{Beta}(1,1)}\text{d}\theta,$$
and we use the Monte Carlo estimator
$$\hat{\mu}_{f(\theta)=\text{Beta}(1,5)} = \frac{1}{N}\sum_{i=1}^N\theta_i\cdot 5(1-\theta_i)^4,$$
where the $\theta_i$ are the samples from ## 2. 
